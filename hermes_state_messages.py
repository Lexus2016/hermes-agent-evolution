...
from agent.message_metadata import normalize_message_origin as _normalize_message_origin
...
_INSERT_MESSAGE_SQL = """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   display_metadata, display_identity, origin)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
...
class SessionMessagesMixin:
    ...
    def _rows_to_conversation(self, rows, *, session_id: str, include_ancestors: bool, repair_alternation: bool,
        ...
        for row in rows:
            ...
            if "origin" in row.keys() and (row_origin := _normalize_message_origin(row["origin"])):
