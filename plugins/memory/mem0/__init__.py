...
def _parse_iso_epoch(value: str) -> Optional[float]:
    """Parse an ISO-8601 ``Z`` timestamp to POSIX epoch. ``None`` on failure.

    Uses ``calendar.timegm`` (NOT ``time.mktime``): the timestamps written by
    ``_now_iso`` are UTC (``Z`` suffix, derived from ``time.gmtime``), so they
    must be interpreted as UTC when converted to an epoch. ``time.mktime``
    interprets the local-timezone, which silently no-ops the staleness filter
    on non-UTC hosts (issue #1289 rework brief, PR #1291 review).
    """
    if not value:
        return None
    try:
        # The format is always ``Z`` (UTC) — parse directly with the ``Z``
        # stripped and ``calendar.timegm`` (UTC epoch) rather than
        # ``time.mktime`` (local epoch) / ``strptime`` ``%z`` dance.
    ...
    def handle_tool_call(self, tool_name: str, args: dict, **kwargs) -> str:
        ...
        if tool_name == "mem0_search":
            ...
            try:
                ...
                # are always filtered (they carry a `supersedes` link).
