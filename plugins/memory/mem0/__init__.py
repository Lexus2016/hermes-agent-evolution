...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def _add(self, messages: list, infer: bool, metadata: dict | None = None):
        meta = {"channel": self._channel} if self._channel else {}
        if metadata:
            meta.update(metadata)
        meta = _temporal_metadata(meta)
        return self._backend.add(messages, user_id=self._user_id, agent_id=self._agent_id, infer=infer, metadata=meta)
    ...
    def _tool_update(self, args: dict) -> str:
        ...
        text = args["text"]
        supersede_meta = {_META_VALID_UNTIL: _now_iso()}
        result = self._backend.update(memory_id, text, metadata=supersede_meta)
        try:
            meta = {"channel": self._channel} if self._channel else {}
            meta = _temporal_metadata(meta, supersedes=memory_id)
            self._backend.add(
