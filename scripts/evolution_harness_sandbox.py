...
def validate_code_diff(
    ...
    return {
        ...
        "applied": applied,
        "gate": {"passed": passed, "exit_code": getattr(gate, "exit_code", None),
...
def main(argv: List[str], *, gate_runner: Optional[GateRunner] = None) -> int:
    ...
    try:
        ...
        print(f"[evolution-harness-sandbox] input is not valid JSON: {exc}", file=sys.stderr)
        return 2
