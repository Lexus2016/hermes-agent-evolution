...
def test_real_binaries_execute_leading_dash_program_payload(
    ...
    if sys.platform == "darwin":
        if tool == "sort":
            pytest.skip("BSD sort on macOS does not support GNU --compress-program")
        if needs_tty:
            pytest.skip("script -qec is util-linux specific; BSD script on macOS uses a different syntax")
