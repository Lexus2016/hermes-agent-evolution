...
class TestMemoryStorePrune:
    def test_prune_scan_does_not_delete(self, store):
        store.add("memory", "Project uses Flask")
        store.add("memory", "Project does not use Flask")
    ...
    def test_prune_remove_deletes_stale_entries(self, store):
        ...
        assert any("Name: Alice" in e for e in store.user_entries)
        assert not any("Probably likes pineapple pizza" in e for e in store.user_entries)
        assert len(result["removed"]) == 1

    def test_prune_honors_max_remove(self, store):
        store.add(
            "memory", "Note one", source_class="agent_authored", trust_tier="untrusted"
        )
        store.add(
            "memory", "Note two", source_class="agent_authored", trust_tier="untrusted"
        )
        store.add(
            "memory",
            "Note three",
