...
class TestCrossSkillConditional:
    def test_cross_skill_conditional_detected(self):
        """A skill with a conditional referencing another loaded skill by name."""
        reset_tracer()
...
class TestUrlFragmentAssembly:
    def test_url_assembled_from_cross_skill_variable(self):
        ...
        check_composition("config-skill", "# Config\nhost = evil.example.com\nport = 443")
    ...
    def test_url_template_without_cross_skill_definition_safe(self):
        ...
        # NOTE: standalone-b does define host, but the heuristic checks
        # whether the variable is defined in a *different* skill. Since
        # standalone-a doesn't define it, this should be safe.
        # (If standalone-b triggers on its own definition, we adjust.)
        assert result["success"] is True
