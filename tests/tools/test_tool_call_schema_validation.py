...
class TestValidateToolArgs:
    ...
    def test_array_and_object_wrong_type(self):
    ...
    def test_optional_and_union_types(self):
        ...
        assert validate_tool_args("u", {"v": 7}, su)[0]
        ok, err = validate_tool_args("u", {"v": True}, su)
        assert not ok and "expected string or integer" in err

    def test_no_schema_unknown_param_and_non_dict(self):
        assert validate_tool_args("any", {"x": 1}, None)[0]
        s = _schema({"q": {"type": "string"}}, ["q"])
        assert validate_tool_args("s", {"q": "hi", "extra": 99}, s)[0]
...
class TestModelToolsDispatch:
    def test_wrong_type_returns_error_json(self, monkeypatch):
        ...
        assert "expected integer" in parsed["error"]