...
def _housekeeping_code_skew(runner, loop) -> None:
    """Check for git revision drift on service-managed hosts and request graceful restart."""
    if runner is None or getattr(runner, "_skew_restart_requested", None) is True:
        return
    try:
        ...
        if skew is not None:
            ...
            if not supports_systemd_services():
                ...
                runner._skew_restart_requested = True
        ...
        logger.debug("code skew check failed: %s: %s", type(exc).__name__, exc)
...
def _start_gateway_housekeeping(
    ...
    chores += [
        ...
        (5, "Code skew check", lambda: _housekeeping_code_skew(runner, loop))]
