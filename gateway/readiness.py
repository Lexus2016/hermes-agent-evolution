...
def collect_runtime_readiness(
    *, configured_model: str, runtime_status: dict[str, Any] | None, active_api_runs: int = 0,
    ...
    # Disk is advisory: a 90%+ volume must not flip the runtime from ok to
    # degraded while state_db/config/model/gateway are healthy.
    _core = ("state_db", "session_store", "config", "model", "gateway")
    return {
        "status": "ok" if all(checks[k].get("status") == "ok" for k in _core) else "degraded",
        "checks": checks,
    }
