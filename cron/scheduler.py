...
# `_count_tool_calls` must sit above `_maybe_cron_refusal_recovery` because
# the recovery path calls it; leaving it at EOF re-creates the NameError
# on a prefix load that reaches `_run_job_impl`.
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower or "create a new one-shot job" in lower:
            remediation = (
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    # final model. cron.thinking (str) then layers
    # cron-specific control on top (#431/#433): thinking is DISABLED by
    # default for cron when cron.thinking is configured to reduce provider timeouts.
    #   "off"/"disabled" -> thinking disabled
    ...
    _c = cfg if isinstance(cfg, dict) else {}
    cron_cfg = _c.get("cron", {}) if isinstance(_c.get("cron", {}), dict) else {}
    if "thinking" in cron_cfg:
        agent_cfg = _c.get("agent", {}) if isinstance(_c.get("agent", {}), dict) else {}
        ...
        elif cron_thinking in ("off", "disabled"):
            return {"enabled": False}
        elif cron_thinking in VALID_REASONING_EFFORTS:
            return {"enabled": True, "effort": cron_thinking}
        else:
            logger.warning(
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    """Precedence: per-job model (persisted in jobs.json) > managed profile override > first-run
    ...
    model = (
        ...
        else job.get("model") or cron_env_setting("HERMES_MODEL") or ""
    )
...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent, agent=None) -> str:
    ...
    if result.get("failed") is True or (result.get("completed") is False and not max_iteration_summary):
        if turn_exit_reason == "loop_guard_cron_hard_stop":
            _stuck_tool = getattr(agent, "_loop_guard_tracked_tool", None) if agent else None
            if _stuck_tool:
                try:
    ...
    if _end_reason == "cron_complete":
        try:
            _db_messages = _session_db.get_messages(
                _final_cron_session_id, include_inactive=True
            )
            if not any(
                isinstance(_m, dict) and _m.get("role") == "tool"
                for _m in _db_messages
            ):
                _end_reason = "cron_noop"
                logger.warning(
...
def _resolve_cron_agent_setup(job: dict, job_id: str, job_name: str, jc) -> _CronAgentSetup:
    ...
    with contextlib.suppress(Exception):
        ...
        stage = evolution_preflight.evolution_job_stage(job)
    agent_owns_routing = bool(stage)
    setup.agent_owns_routing = agent_owns_routing
    ...
    from hermes_cli.config import cron_model_drift_guard_enabled, cron_model_drift_axes
    if cron_model_drift_guard_enabled(_cfg):
        configured_provider_for_drift = (
            str((_cfg.get("model") or {}).get("provider") or "").strip().lower()
        ...
        _current_provider = str(
            ...
            or (setup.runtime.get("provider") or "")
        ).strip().lower()
        _current_model = str(jc.model or setup.model or "").strip().lower()
        _drift: list[str] = []
        ...
        if _drift:
            ...
            if _auto_repin_done:
                ...
                if "provider" in _drift and _current_provider:
                    job["provider_snapshot"] = _current_provider
                if "model" in _drift and _current_model:
                    job["model_snapshot"] = _current_model
            else:
    ...
    setup.credential_pool = None if agent_owns_routing else _load_credential_pool(setup.runtime, job_id)
...
def _construct_cron_agent(AIAgent, job: dict, _cfg: dict, setup: _CronAgentSetup, *, workdir, session_id, session_db):
    runtime = setup.runtime or {}
    pr = _cfg.get("provider_routing") or {}
    agent_owns_routing = setup.agent_owns_routing
    configured_provider_for_drift = (
        str((_cfg.get("model") or {}).get("provider") or "").strip().lower()
        if isinstance(_cfg.get("model"), dict)
        else ""
    ...
    return AIAgent(
        ...
        request_overrides=None if agent_owns_routing else runtime.get("request_overrides"),
...
def _run_job_impl(
    ...
    # cycles. Checked here, before SessionDB/AIAgent construction.
    from cron import evolution_preflight
    ...
    try:
        ...
        # The prompt was assembled and all pre-agent gates passed. The diversion
        # directive (if any, built by _inject_last_hard_stop_directive in
        # _build_job_prompt above) is final and the agent run is definitely
        ...
        # never actually reaches the agent (see the early returns above). (#720)
        ...
        final_response = _final_response_from_result(result, job_id, job_name, AIAgent, agent=agent)
...
def _compose_run_delivery(
...
) -> tuple[str, bool, bool, bool, bool, bool, Optional[str]]:
    """Text to deliver for a finished run. Returns ``(deliver_content, blocked_config,
    balance_low, drift_skip, silent_alert, incident_acked, failure_incident_id)``; ``silent_alert``: an alert-once marker
    ...
    balance_low_silent = BALANCE_LOW_SILENT_MARKER in err
    balance_low = balance_low_silent or BALANCE_LOW_MARKER in err
    ...
    if (blocked_config or balance_low) and not success:
        # Blocked-config alert: bypass the generic failure summarizer (its auth/timeout
        # heuristics would mislabel this as a provider runtime failure) — say plainly that
        # config validation blocked the run and nothing was spent.
        _pf_text = re.sub(
            r"\[(?:blocked_config|balance_low)[^\]]*\]\s*", "", err
        ).strip()
        if balance_low:
            deliver_content = (
    ...
    return (
        ...
        balance_low,
        drift_skip,
        (blocked_config_silent or balance_low_silent or drift_skip_silent),
        incident_acked,
        failure_incident_id,
    )
...
def _save_compose_deliver(
    ...
    (
        deliver_content, d.blocked_config, d.balance_low, d.drift_skip, _silent_alert, d.incident_acked, d.failure_incident_id,
...
def _finish_completed_run(d: _RunDelivery, fire_owner: Optional[str], execution_id: str) -> bool:
    ...
    if not d.success:
        try:
            from cron.jobs import save_job_failure
            save_job_failure(
                job,
                success=False,
                error=d.error,
            ...
            logger.warning("Job '%s' failure record saved to cron/failures", job.get("id"))
...
def _run_one_job_body(
    ...
    try:
        ...
        d = _RunDelivery(job=job, success=success, error=error, output=output)
        try:
...
from cron.scheduler_preflight import (  # noqa: E402
    BLOCKED_CONFIG_MARKER, BLOCKED_CONFIG_SILENT_MARKER,
    BALANCE_LOW_MARKER, BALANCE_LOW_SILENT_MARKER,
    _cron_preflight_enabled, _empty_requested_mcp_toolsets,
    _is_transient_provider_resolve_error, _preflight_job_config,
    _preflight_check_provider_key, _preflight_check_provider_balance,
)
