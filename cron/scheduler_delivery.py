...
def _resolve_origin(job: dict) -> Optional[dict]:
    ...
    """
    try:
        sched_resolve = getattr(_sched, "_resolve_origin", None)
        if sched_resolve is not None and sched_resolve is not _resolve_origin:
            return sched_resolve(job)
    except NameError:
        pass
...
def _deliver_result(
    ...
    *,
    ...
    # content. Error deliveries (success=False / for_failure=True) always pass
    # through untouched so silent/result_only/summary can never swallow a failure alert.
    user_cfg = None
    with contextlib.suppress(Exception):
        user_cfg = _sched.load_config()
    verbosity_cfg = user_cfg or {}
    resolve_fn = getattr(_sched, "_resolve_delivery_verbosity", None)
    apply_fn = getattr(_sched, "_apply_delivery_verbosity", None)
    if resolve_fn and apply_fn:
        _verbosity = resolve_fn(
            job, [t.get("chat_id") for t in targets], verbosity_cfg
        )
        _shaped = apply_fn(_verbosity, content, success=success)
        if _shaped is None:
            logger.info(
