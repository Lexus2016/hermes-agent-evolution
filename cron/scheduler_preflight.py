...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        if evolution_preflight.evolution_job_stage(job):
            return None  # evolution stages manage their own routing and preflight
    except Exception:
        pass
...
def _preflight_check_provider_balance(job: dict, cfg: dict) -> Optional[str]:
    ...
    HTTP 402 Insufficient Balance is non-retryable billing exhaustion.
    """
    try:
        from hermes_cli.fallback_config import get_fallback_chain
        if get_fallback_chain(cfg):
    ...
    try:
        ...
        from cron import evolution_preflight
        from hermes_cli.runtime_provider import resolve_runtime_provider
        from hermes_constants import get_hermes_home as _get_hermes_home_default
        ...
        _get_home = getattr(_sched, "_get_hermes_home", None)
        hermes_home = _get_home() if _get_home else _get_hermes_home_default()
...
def _preflight_job_config(job: dict, cfg: dict) -> Optional[str]:
    ...
    """
    try:
        check_key = getattr(_sched, "_preflight_check_provider_key", _preflight_check_provider_key)
        check_bal = getattr(_sched, "_preflight_check_provider_balance", _preflight_check_provider_balance)
    except NameError:
        check_key = _preflight_check_provider_key
        check_bal = _preflight_check_provider_balance
    for name, check in (
        ("provider_key", lambda: check_key(job, cfg)),
        ("provider_balance", lambda: check_bal(job, cfg)),
