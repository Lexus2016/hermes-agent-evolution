...
def _activate_job_record(job: Dict[str, Any]) -> None:
    ...
    # #70: activating a job is an operator re-enable — clear the failure streak
...
_UPDATE_FIELD_NORMALIZERS: Dict[str, Callable[[Any], Any]] = {
    ...
    "approval_mode": lambda v: None if v in {None, "", False} else _normalize_approval_mode(v),
    "delivery_verbosity": lambda v: None if v in {None, "", False} else _normalize_delivery_verbosity(v),
    "monitor_script": _normalize_job_optional_text,
    "monitor_url": _normalize_job_optional_text,
...
def create_job(
    ...
    if pin_inference and not f["no_agent"]:
        if f["provider"] is None and provider_snapshot:
            f["provider"] = provider_snapshot
            provider_snapshot = None
        if f["model"] is None and model_snapshot:
    ...
    for key, value in (
        ...
        ("delivery_verbosity", normalized_delivery_verbosity),
...
def _record_run_outcome(
    ...
    status: Optional[str], now: str, tool_calls: Optional[int] = None,
) -> None:
    ...
    def apply(jobs, _i, job):
        if expected_fire_owner is not None:
            ...
            if not isinstance(claim, dict) or claim.get("by") != expected_fire_owner:
                ...
                return False
        now = _hermes_now().isoformat()
        _record_run_outcome(job, success, error, delivery_error, status, now, tool_calls=tool_calls)
        ...
        if not success and model_unreachable and not is_terminal_job(job):
            # Transient network/DNS: bounded re-run instead of waiting a full period.
            plan_retry(job)
        else:
            ...
            # Unreachable retries own next_run_at via plan_retry above — do not
            # also skip slots or auto-pause that path.
            if not success and status not in _SYNTHETIC_FAILURE_STATUSES:
                _skips = int(job.get("failure_backoff_skips") or 0)
                if _skips > 0 and job.get("next_run_at") is not None:
