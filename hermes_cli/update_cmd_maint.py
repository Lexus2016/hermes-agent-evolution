...
def _resolve_pre_update_snapshot_max_file_size() -> int:
    ...
    raw = _load_updates_cfg().get("pre_update_snapshot_max_file_size_mb")
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    cap = _resolve_pre_update_snapshot_max_file_size()
    snapshot_id = create_quick_snapshot(
        label="pre-update", keep=_PRE_UPDATE_SNAPSHOT_KEEP, max_file_size=cap,
    )
    if snapshot_id:
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        ...
        _sibling_snaps = create_pre_update_snapshots_all_profiles(
            keep=_PRE_UPDATE_SNAPSHOT_KEEP, max_file_size=cap,
        )
        if _sibling_snaps:
