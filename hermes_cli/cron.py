...
def cron_create(args):
    ...
    _paused = getattr(args, "paused", None)
    _paused_reason = getattr(args, "paused_reason", None)
    _is_mock = hasattr(_paused, "_mock_return_value") or hasattr(_paused_reason, "_mock_return_value")
    _paused_kwargs = {}
    if not _is_mock and (_paused or _paused_reason is not None):
        _paused_kwargs = {"paused": _paused, "paused_reason": _paused_reason}
    result = _cron_api(
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: hermes cron [list|create|edit|pause|resume|run|remove|repin|resnap|status|runs|doctor|tick]")
    sys.exit(1)
