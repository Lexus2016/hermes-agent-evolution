...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    # MCP plaintext-secret / injection audit (issue #91)
    ...
    if total == 0:
        if output_json:
            ...
            print("No components discovered (everything skipped, or empty environment).")
    ...
    if any(SEVERITY_ORDER.get(f.vuln.severity, 0) >= threshold for f in findings):
        return 1
    if mcp_findings:
        return 1
    return 0
