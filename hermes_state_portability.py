...
class SessionPortabilityMixin:
    ...
    def _import_session_row(
        ...
        *,
        ...
        sanitized_messages = []
        for msg in messages:
            clean = {**msg, **{key: _json_value(msg.get(key)) for key in _IMPORT_MESSAGE_JSON_FIELDS}}
        ...
        def _do(conn):
            ...
            for item in normalized:
                ...
                if conn.execute("SELECT 1 FROM sessions WHERE id = ? LIMIT 1", (session_id,)).fetchone():
                    ...
                    continue
                self._import_session_row(conn, raw, item["messages"], session_id, trust_origin=trust_origin)
