...
from tools.file_operations_search import SearchMixin, _enrich_search_parse_error
...
# Echoed when the path is a regular file that cannot be read (permission denied).
...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _probe_regular_file(self, path: str) -> tuple[int, str]:
        ...
        stat_result = self._exec(
            f"if [ -f {arg} ]; then "
            f"if [ ! -r {arg} ]; then echo {UNREADABLE_SENTINEL}; "
            f"else wc -c < {arg} 2>/dev/null; fi; "
        ...
        if stat_output == UNREADABLE_SENTINEL:
            return 0, "unreadable"
        if stat_result.exit_code != 0:
            # Non-empty unexpected stdout (no sentinel) means the shell itself
            # never ran the probe (dead container / transport down). Empty
            # stdout is a mocked or real miss — diagnose missing/dir/permission.
            if stat_output:
                return 0, "env_unavailable"
            diag_err, _ = self._diagnose_read_failure(path)
            diag_low = diag_err.lower()
            if "directory" in diag_low:
                return 0, "not_regular"
            if "permission" in diag_low:
                return 0, "unreadable"
            if "not found" in diag_low:
                return 0, "missing"
            return 0, "env_unavailable"
        try:
    ...
    def _diagnose_read_failure(self, path: str) -> tuple[str, list[str]]:
        """Diagnose a read failure by checking existence, directory, permissions, or suggesting similar files (#1326, #1488, #1587)."""
    ...
    def read_file_raw(self, path: str) -> ReadResult:
        ...
        file_size, status = self._probe_regular_file(path)
        if status == "missing" or status == "unreadable":
            err, sims = self._diagnose_read_failure(path)
            return ReadResult(error=err, similar_files=sims)
        if status == "not_regular":
            return self._not_regular_error(path)
    ...
    def _no_match_result(self, path: str, content: str, old_string: str,
                         new_string: str, match_count: int, error: Optional[str],
                         strategy: Optional[str] = None) -> PatchResult:
        ...
        Otherwise attach a structured diagnostic so the model can self-correct
        without a re-read; fall back to a "Did you mean?" snippet if none."""
        from tools.fuzzy_match import (
            format_no_match_hint, format_structured_error, is_already_applied,
        )
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        read_result = self._cat(path)
        if read_result.exit_code != 0:
            # #1326 — The old cat swallows the reason the read failed. Probe
            # distinctly so not_found, directory, and permission errors classify
            # cleanly and carry appropriate recovery hints.
            err, sims = self._diagnose_read_failure(path)
            return PatchResult(error=err, similar_files=sims)
        ...
        if error or match_count == 0:
            # Identical old/new present in the file is a landed no-op
            # (success + no_change); identical but absent still errors.
