...
class ShellFileOperations(FileOperations):
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 2000) -> ReadResult:
        ...
        try:
            ...
            wc_rc = int(status[2]) if len(status) > 2 else 0
        ...
        if read_rc != 0 or wc_rc != 0:
            err, sims = self._diagnose_read_failure(path)
            return ReadResult(error=err or "Failed to read file", similar_files=sims)
    ...
    def _read_file_sequential(self, path: str, offset: int, limit: int) -> ReadResult:
        ...
        if read_result.exit_code != 0:
            err, sims = self._diagnose_read_failure(path)
            return ReadResult(error=err or f"Failed to read file: {read_result.stdout}", similar_files=sims)
