...
def _staleness_score(entry: str, meta: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
    ...
    age_score = (
        min(days_since_recall, 365) * _AGE_WEIGHT
        + min(days_since_added, 365) * _AGE_ADDED_WEIGHT
    )
...
class MemoryStore:
    ...
    def prune(
        ...
        contradiction_texts = {a.strip() for a, _, _ in contradictions} | {
            b.strip() for _, b, _ in contradictions
        }
        ...
        remove_limit = (
            max_remove if max_remove is not None else max(1, len(entries) // 5)
        )
        ...
        if mode != "remove":
            return {
                "success": False,
                "error": f"Invalid prune mode '{mode}'. Use 'scan' or 'remove'.",
            }
        ...
        for _score, _i, text, _details in scored[:remove_limit]:
            ...
            if _score <= 0:
                continue
            # Contradictions are surfaced for review; do not auto-delete them.
            if text in contradiction_texts:
                continue
...
def memory_tool(
    ...
    if action == "add":
        ...
        return tool_error(
            f"Unknown action '{action}'. Use: add, replace, remove, search, prune",
            success=False,
        )
