...
PROCESS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            "action": {
                "type": "string",
                "enum": ["list", "poll", "log", "wait", "kill", "stop", "write", "submit", "close", "handoff"]
            },
            "session_id": {
                "type": "string",
...
_SESSION_ACTIONS = {
    ...
    "stop": (lambda sid, a: process_registry.kill_process(sid), True),
...
def _handle_process(args, **kw):
    ...
    if action == "list":
        return json.dumps(_list_processes(task_id), ensure_ascii=False)
    if action == "handoff":
        if not session_id:
    ...
    if action in _SESSION_ACTIONS:
        ...
        payload = _redact_process_result(result) if redact else result
        try:
            return json.dumps(payload, ensure_ascii=False)
        except TypeError:
            # poll()/wait() mocks or unexpected objects must not leak into
            # the tool result — the model (and tests) need JSON.
            return tool_error(
                f"No process with ID '{session_id}'. "
                "No processes are currently registered. "
                "Use action='list' to verify."
    ...
    valid_actions = {"list", "handoff"} | set(_SESSION_ACTIONS.keys())
