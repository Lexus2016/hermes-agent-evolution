...
from tools.skill_manager_guards import (
    ...
    _org_mirror_write_guard, _pinned_guard, _validate_delete_target, _is_background_review,
    _reset_background_review_read_marks, _is_skill_md_target, _refusal as _err)
...
_FLAT_OP_KEYS = ("content", "category", "file_path", "file_content", "old_string", "new_string",
                 "absorbed_into", "operations", "rationale")
...
def _record_success(action, name, result, *, file_path, absorbed_into, task_id,
                    session_id, ledger_before, rationale: Optional[str] = None) -> None:
        ...
        if action == "create":
            ...
            if is_background_review():
                with suppress(Exception):
                    ...
                    if run_id:
                        from tools.skill_usage import set_source_run_id
...
def skill_manage(
    ...
    session_id: str = None, rationale: str = None, operations=None) -> str:
    ...
    if result.get("success"):
        from tools.skill_provenance import is_background_review
        if action == "create" and is_background_review() and not _skill_gate_bypass.get():
            validation_error = validate_skill_content(name)
            if validation_error:
                logger.warning(
                    "Validation gate BLOCKED auto-created skill %r: %s",
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        f"{name!r}: {validation_error} The skill was "
            ...
            if not _ok:
                logger.warning(
                    "Provenance gate BLOCKED promotion of %r: %s",
                    name, _prov_reason,
                )
                _rollback_skill_dir(name)
                return json.dumps({
                    "success": False,
                    "error": (
                        f"Provenance gate rejected skill {name!r}: "
