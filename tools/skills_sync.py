...
def _recover_renamed_skill(st: "_SyncState", skill_name: str, dest: Path) -> Optional[str]:
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        _drop_remaining_stale_copies(skill_name, origin_hash, dest, st.active_index, st.hub_paths, st.quiet)
        return rel
    return None
...
def _drop_remaining_stale_copies(
    ...
    origin_hash: str,
    dest: Path,
    active_index: Optional[Dict[str, List[Path]]],
    hub_paths: Set[str],
    quiet: bool,
) -> None:
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    if not bundled_dir.exists():
        return {"copied": [], "updated": [], "skipped": 0, "user_modified": [], "cleaned": [],
                "suppressed": [], "relocated": [], "total_bundled": 0, "optional_provenance_backfilled": []}
        ...
        if skill_name in external_index:
            ...
            for old_copy in stale_copies:
                if origin_hash and _dir_hash(old_copy) == origin_hash:
                    try:
                        ...
                        logger.debug("Could not remove stale copy %s", old_copy, exc_info=True)
            ...
            if kept_modified:
                ...
                st.say(
                    ...
                    f"{dest.relative_to(_skills_dir())}, but your modified "
                    f"copy at {', '.join(str(p.relative_to(_skills_dir())) for p in kept_modified)} "
                    "was kept — not relocating"
                )
            else:
                try:
                    dest.parent.mkdir(parents=True, exist_ok=True)
                    ...
                    st.manifest[skill_name] = bundled_hash
                    st.relocated.append(skill_name)
                    st.say(f"  → {skill_name} (relocated to {dest.relative_to(_skills_dir())})")
                except (OSError, IOError) as e:
                    st.say(f"  ! Failed to relocate {skill_name}: {e}")
