...
class MCPServerTransportMixin:
    ...
    async def _serve_session(self, session, connect_timeout: float,
        ...
        if getattr(self, "_detect_stateless_support", lambda: False)():
...
def _govern_outbound_endpoint(server_name: str, url: str) -> None:
    ...
    if not url:
        return
    try:
        from evolution.lib.shadow_mcp import DENY, ShadowMcpDeniedError, get_governor
        verdict = get_governor().record_contact(server_name, url)
        if verdict == DENY:
            raise ShadowMcpDeniedError(
                f"MCP server '{server_name}' endpoint {url} is blocked by the shadow_mcp deny policy"
            )
    except ShadowMcpDeniedError:
        raise
    except Exception:
        logger.debug("shadow-mcp governance check failed for '%s'", server_name, exc_info=True)
