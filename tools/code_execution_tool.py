...
def execute_code(
    ...
    res_str = execute_in_session_kernel(
    ...
    try:
        ...
        if isinstance(res_data, dict) and (res_data.get("status") in ("error", "timeout") or res_data.get("exit_code", 0) != 0):
            if "classification" not in res_data:
                diag = _classify_execution_failure(
                    exit_code=res_data.get("exit_code", -1),
                    stderr_text=res_data.get("stderr", "") or res_data.get("error", ""),
                    status=res_data.get("status", "error"),
                    timeout_value=_cfg.get("timeout", DEFAULT_TIMEOUT),
                )
                res_data.update({k: v for k, v in diag.items() if k not in res_data})
                res_str = json.dumps(res_data, ensure_ascii=False)
    except Exception:
        pass
    return res_str
