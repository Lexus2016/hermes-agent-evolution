...
def is_already_applied(content: str, old_string: str, new_string: str) -> bool:
    ...
    Identical old/new is a landed no-op at any length; the >=8-char guard is
    only for the heuristic (old gone, new present) so a short coincidence
    cannot mask a genuine miss."""
...
def classify_error(error: Optional[str], strategy: Optional[str] = None) -> str:
...
def _format_file_context_snippet(
    ...
    # Find the content line with the highest similarity to the anchor line
    best_idx = -1
    best_ratio = 0.0
    for i, line in enumerate(content_lines):
        stripped = line.strip()
