...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "WORKFLOW HIERARCHY (cheapest first — select > generate > edit):\n"
        "- SELECT: before spawning a worker for a need, check whether an "
        "existing skill, plugin, or a previous child's proven configuration "
        "already covers it; reuse it.\n"
        "- GENERATE: spawn a new child (with a clear goal + context) only "
        "when nothing existing covers the need.\n"
        "- EDIT mid-execution (changing a running child's plan) only under "
        "genuine uncertainty — never as routine.\n\n"
        "RULES:\n"
