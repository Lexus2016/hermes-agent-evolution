...
from tools.cronjob_prompt_scan import (
    _scan_cron_prompt,
    _is_script_using_zwj,
    _zwj_has_script_neighbour,
)
...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    try:
        job = create_job_with_scheduler_registration(
            ...
            pin_inference=bool(a.get("pin_inference")),
            # CLI-only lane: absent from CRONJOB_SCHEMA and the model dispatch (models don't pick models).
            reasoning_effort=a["reasoning_effort"],
            failure_deliver=_resolve_cron_context_deliver(_normalize_deliver_param(a["failure_deliver"])),
            approval_mode=a.get("approval_mode"),
            delivery_verbosity=a.get("delivery_verbosity"),
...
def _update_run_fields(job: Dict[str, Any], a: Dict[str, Any], updates: Dict[str, Any]) -> Optional[str]:
    ...
    if a.get("approval_mode") is not None:
        updates["approval_mode"] = a["approval_mode"]
    if a.get("delivery_verbosity") is not None:
        updates["delivery_verbosity"] = a["delivery_verbosity"]
    ...
    try:
        ...
        if handler is not None:
            ...
            try:
                ...
                if isinstance(_res_obj, dict) and _res_obj.get("success"):
...
_HANDLER_FORWARDED_ARGS = (
    ...
    "approval_mode", "delivery_verbosity", "paused_reason", "all")
