...
from tools.web_tools_rescue import _keyless_rescue_enabled, _rescue_eligible, _rescue_search
...
_web_search_default_key = "_default_"
...
def _get_extract_char_limit() -> int:
    return _effective_char_limit(None)


def web_search_tool(query: str, limit: int = 5, session_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        threshold = _get_web_search_streak_threshold()
        if threshold > 0 and streak >= threshold and response_data.get("success"):
