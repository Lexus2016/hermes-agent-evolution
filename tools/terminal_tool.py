...
def get_active_env(task_id: str):
    """Return the cached env for *task_id*, or None.

    Facade patch target (``tools.terminal_tool.get_active_env``). Tests and
    in-tree callers bind here; do not late-import the lifecycle sibling.
    """
    with _env_lock:
        return _lookup_active_env(_resolve_container_task_id(task_id), task_id)


def _nearby_missing_path_hint(command: str, output: str) -> Optional[str]:
    """#2242 — 'Did you mean' snippet when the shell reports a missing path."""
    nf_path = None
    m_access = re.search(r"cannot access ['\"]([^'\"]+)['\"]", output or "")
    m_nsf = re.search(
        r":\s+'?([^\s':]+?)'?\s*:\s*No such file or directory",
        output or "",
    )
    if m_access:
        nf_path = m_access.group(1)
    elif m_nsf:
        nf_path = m_nsf.group(1)
    elif command and (
        "No such file or directory" in (output or "")
        or "not found" in (output or "").lower()
        or "command was not found" in (output or "").lower()
    ):
    ...
    if not nf_path:
        return None
    nearby = suggest_nearby_paths(nf_path)
    return format_nearby_hint(nf_path, nearby)


def _apply_classified_enrichments(
    result_dict: dict, result: dict, command: str, output: str,
) -> None:
    """Attach truncation spill + nearby-path hint on classified non-zero exits."""
    from tools.terminal_tool_result import _redact_spill_file
    for key, value in _redact_spill_file(
        result.get("full_output_path"), result.get("output_total_chars"), command,
    ):
        if value is not None:
            result_dict[key] = value
    hint = _nearby_missing_path_hint(command, output)
    if hint:
        result_dict["error"] = str(result_dict.get("error") or "") + "\n\n" + hint
...
_SHELL_BUILTINS = frozenset({
    "cd", "pwd", "echo", "printf", "export", "unset", "source", ".",
    "test", "[", "[[", "true", "false", "exec", "exit", "command",
    "builtin", "type", "alias", "unalias",
})
...
def _run_approval_guards(command: str, env_type: str, config: Dict[str, Any], *, force: bool) -> _ApprovalVerdict:
    ...
    if not approval["approved"]:
        if approval.get("status") == "pending_approval":  # gateway ask mode
            # Cron/headless: nobody can answer a pending approval. Returning
            # pending_approval with an empty error makes the agent retry the
            # same command (#1542). Convert to a non-retryable blocked error.
            from tools.approval import (
                _is_interactive_cli,
        ...
        raise _Rejected(_error_json(block_error, status="blocked"))
...
def _acquire_env(plan: _ExecPlan, task_id: Optional[str]) -> Any:
    ...
    # Facade ``get_active_env`` is the monkeypatch seam (tests patch
    # ``tools.terminal_tool.get_active_env``). Do not call the lifecycle
    # sibling here — that misses the patch.
    env: Any = get_active_env(task_id or eff)
    if env is not None:
        return env
...
def _run_foreground(
    ...
    while True:
        try:
            ...
            # Only pass yield_handler if the environment's execute() accepts it (#647)
            yk = _yield_kwargs(command, env_type=env_type, cwd=command_cwd, effective_task_id=eff,
                                task_id=task_id, session_key=session_key)
            if yk and "yield_handler" in yk:
                import inspect
                try:
                    sig = inspect.signature(env.execute)
                    if "yield_handler" not in sig.parameters and not any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                        yk = {}
                except Exception:
                    pass
            result = env.execute(
            ...
            if classification.category in {
                ...
                result_dict = {
                    ...
                    "failure_class": classification.category.value if classification.category != FailureCategory.unknown else "timeout",
        ...
        if returncode != 0:
            ...
            if classification.category not in {
                ...
                if rec:
                    result_dict["recommendation"] = rec
                _apply_classified_enrichments(result_dict, result, command, output)
                return json.dumps(result_dict, ensure_ascii=False)
            else:
                _note_terminal_failure(task_id, command, returncode, output)
        else:
...
def terminal_tool(
    ...
    if not isinstance(command, str):
        return json.dumps({
            "output": "",
            "exit_code": -1,
            "error": f"command expected string, got {type(command).__name__}",
            "status": "error",
        }, ensure_ascii=False)
    try:
        ...
        # Preflight guard (#3243): fail fast on common deterministic errors before shell execution
        from tools.environments.local import LocalEnvironment
        is_mock_env = env is not None and not isinstance(env, LocalEnvironment)
