...
def _trust_gate_check(server_name: str, tool_name: str) -> Optional[str]:
    """Approval gate for untrusted/sensitive servers. None to proceed, else a ``tool_error``.
    Fail-closed: approval-system errors block. ``sensitive`` gates every call; ``untrusted``
    skips tools whose discovery-time ``readOnlyHint`` is exactly True."""
    trust = _core._server_trust_levels.get(server_name, _core._TRUST_FULL)
    if trust == _core._TRUST_FULL:
        return None
    if trust == _core._TRUST_UNTRUSTED and (
            _core._tool_read_only_hints.get(server_name, {}).get(tool_name) is True):
        return None
...
async def _call_tool_racing_stdio_death(server, server_name: str, tool_name: str, args: dict):
    ...
    _meta = None
    _get_meta = getattr(server, "stateless_routing_meta", None)
    if callable(_get_meta):
        try:
            _meta = _get_meta("tools/call", tool_name)
        except Exception:
            _meta = None
...
def _render_call_tool_result(result, server_name: str, tool_name: str = "") -> str:
    ...
    if mcp_field(result, "is_error", "isError", False):
        error_text = _error_result_text(result) or "MCP tool returned an error"
        # #1393 — enrich not_found / validation errors with a recovery directive.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        # Quota-exhaustion short-circuit (#2193)
        if _quota_cooldown_active(server_name):
            remaining = max(
                1,
                int(_server_quota_exhausted_until.get(server_name, 0.0) - time.monotonic()),
            )
            return tool_error(
                f"MCP server '{server_name}' is quota-exhausted "
        ...
        async def _call():
            ...
            return _render_call_tool_result(result, server_name, tool_name=tool_name)

        def _on_failure(exc):
            if _is_quota_error(exc):
                _mark_quota_exhausted(server_name)
                logger.warning(
                    "MCP server '%s' quota-exhausted (HTTP 402/billing), cooldown %ds: %s",
                    server_name, int(_QUOTA_COOLDOWN_SEC), exc,
                )
            _core._bump_server_error(server_name)
...
_make_get_prompt_handler_raw = _make_utility_handler(
...
def _make_get_prompt_handler(server_name: str, tool_timeout: float):
    inner = _make_get_prompt_handler_raw(server_name, tool_timeout)
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        if "unknown prompt" not in err.lower():
            return result
        from tools import mcp_tool as _origin
        extra = _origin._enrich_missing_prompt_error(
            _origin._servers.get(server_name), args.get("name") or "", server_name, tool_timeout)
        payload["error"] = (err + " " + extra.get("hint", "")).strip()
