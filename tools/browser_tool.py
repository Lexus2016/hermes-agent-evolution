...
# Seams for tests that monkeypatch browser_tool internals
def _run_browser_command(session_id: str, command: str, args: Optional[list] = None) -> Dict[str, Any]:
    return _session._run_browser_command(session_id, command, args)

def _last_session_key(task_id: str) -> str:
    return _session._last_session_key(task_id)

def _enforce_browser_eval_policy(expression: str) -> Optional[str]:
    return _eval_policy._enforce_browser_eval_policy(expression)

def _eval_ssrf_guard_active(effective_task_id: str) -> bool:
    return _eval_policy._eval_ssrf_guard_active(effective_task_id)
...
def _failed_response(result: Dict[str, Any], default_error: str) -> str:
    err_dict = _err(result.get("error", default_error))
    _annotate_browser_failure(err_dict)
    return _json_with_fallback(err_dict, result)
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    if result.get("success"):
        ...
        response = _annotate_browser_failure(_err(result.get("error", f"Failed to type into {ref}")))
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if expression is not None:
        ...
        if policy_error:
            resp = _err(policy_error)
            _annotate_browser_failure(resp)
            return _dumps(resp)
...
def _eval_failure_response(result: Dict[str, Any]) -> str:
    ...
    resp = _err(err)
    _annotate_browser_failure(resp)
    return json.dumps(_lp._copy_fallback_warning(resp, result))
