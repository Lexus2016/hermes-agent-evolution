...
class MCPServerTask(MCPServerRunMixin, MCPServerTransportMixin, MCPServerHealthMixin):
    ...
    __slots__ = (
        ...
        "_ever_connected", "_last_park_error", "_stateless_enabled", "_sse_fallback")
    ...
    def __init__(self, name: str):
        ...
        # subsequent identical park-warnings are downgraded to DEBUG (#1391).
    ...
    def _detect_stateless_support(self) -> bool:
        ...
        """
        return getattr(self, "_stateless_enabled", False)
    ...
    def _get_session_id_stateless(self, _get_session_id) -> Optional[str]:
        ...
        B-2 / #1512.
        """
        if getattr(self, "_stateless_enabled", False):
            return None
        try:
...
_TRUST_FULL, _TRUST_UNTRUSTED, _TRUST_SENSITIVE = "full", "untrusted", "sensitive"
...
# ---------------------------------------------------------------------------
# Fork extensions: noise suppression, preflight validation & scanning
# ---------------------------------------------------------------------------
import re
from tools.mcp_tool_errors import NonMcpEndpointError, MissingMcpCommandError
...
def _ensure_stdio_command_resolvable(command: str, env: dict) -> None:
    ...
    if resolved is None:
        raise MissingMcpCommandError(
            ...
            f"Install the server (e.g. ) or add its directory "
...
_MCP_INJECTION_PATTERNS = [
    (re.compile(r"ignore\s+(?:all\s+|previous\s+|above\s+)*instructions", re.I), "prompt_injection"),
...
# Re-exports for test compatibility and facade surface
from tools.mcp_tool_errors import _classify_mcp_failure  # noqa: F401,E402
from tools.mcp_tool_transport import _govern_outbound_endpoint  # noqa: F401,E402
from tools.mcp_tool_registration import (  # noqa: F401,E402
    _register_server_tools,
    _resolve_durability_backend,
    _wrap_with_durability,
    _normalize_server_trust,
)
from tools.mcp_tool_handlers import (  # noqa: F401,E402
    _make_tool_handler,
...
from tools.mcp_tool_loop import _run_on_mcp_loop  # noqa: F401,E402
