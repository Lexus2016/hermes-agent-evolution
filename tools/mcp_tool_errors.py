...
class MissingMcpCommandError(NonMcpEndpointError):
    """Raised when a stdio MCP server's command cannot be found or executed.
    Non-retryable: every attempt fails the same way, so backoff is skipped."""
...
def _classify_mcp_failure(exc: BaseException) -> str:
    ...
    try:
        from evolution.lib.shadow_mcp import ShadowMcpDeniedError
