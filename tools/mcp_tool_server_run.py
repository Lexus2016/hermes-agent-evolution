...
class MCPServerRunMixin:
    ...
    async def _on_transport_error(self, exc: Exception, budget: "_RetryBudget") -> bool:
        ...
        if self._reconnect_retries > _core._MAX_RECONNECT_RETRIES:
            ...
            error_str = f"{type(root).__name__}: {root}"
            if getattr(self, "_last_park_error", None) != error_str:
                ...
                logger.debug(
                    "MCP server '%s' still parked (same error as last probe, suppressed from WARNING): %s",
                    self.name, error_str,
                )
