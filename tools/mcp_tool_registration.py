...
def _normalize_server_trust(value: Any) -> str:
    ...
    if text in (_core._TRUST_FULL, _core._TRUST_UNTRUSTED, getattr(_core, "_TRUST_SENSITIVE", "sensitive")):
        return text
    logger.warning(
...
def _register_server_tools(name: str, server: "MCPServerTask", config: dict) -> List[str]:
    ...
    candidates = [
        _Candidate(c.registry_name, c.origin, c.schema,
                   _wrap_with_durability(c.registry_name, c.handler, backend))
        for c in candidates
    ]
