    ...
    def _run_rg_native(self, argv: List[str], fetch_limit: int, timeout: int,
        ...
        while True:
            ...
            if time.monotonic() > deadline:
                exit_code = 124
                break
        # Timeout/interrupt: SIGKILL first so the drain thread sees EOF. Closing
        # the pipe while the child is still alive deadlocks the close against the
        # blocked reader until the child exits. Do not wait() or killpg.
