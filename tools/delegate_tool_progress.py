...
def _build_child_system_prompt(
    ...
    max_spawn_depth: int = 2, child_depth: int = 1, denied_toolsets: Optional[List[str]] = None,
    ...
    ``denied_toolsets`` (#648): toolsets the delegating call explicitly asked for that are missing from the
    FINAL resolved child toolset list.
    """
    ...
    if attribution:
        try:
            from tools.delegation_attribution import attribution_prompt_block
            parts.append(attribution_prompt_block(attribution))
        except Exception:
            parts.append(f"\nATTRIBUTION:\n{attribution}")
    return "\n".join(parts)
