...
_BILLING_PATTERNS = (
    ...
    "model_not_supported_on_free_tier", "not available on the free tier", "requires a paid subscription", "paid subscription",
)
...
_OVERLOADED_PATTERNS = (
    ...
    "server is temporarily unavailable", "temporarily unavailable",
    "service unavailable", "service is unavailable", "bad gateway", "gateway timeout",
    "upstream connect error", "upstream request timeout", "engine is overloaded",
    "model is currently overloaded", "model is overloaded", "high traffic", "temporarily high load",
)
...
_MODEL_NOT_FOUND_PATTERNS = (
    ...
    "model not supported", "model is deprecated", "model has been deprecated",
    "model is decommissioned", "model has been disabled", "model is disabled",
    "model is not accessible", "not permitted to access this model", "access to model denied",
)
...
_INVALID_MESSAGE_BODY_PATTERNS = (
    ...
    "tool_calls': empty array", "tool_calls: empty array", "expected an array with minimum length 1",
)
...
_REQUEST_VALIDATION_PATTERNS = (
    ...
    "extra_forbidden", "extra fields not permitted", "missing required argument",
    "validation error for", "input should be a valid", "value is not a valid",
    "invalid schema for function", "invalid json schema", "tools schema is invalid",
    "unprocessable entity", "error code: 422",
)
...
_MESSAGE_HEAD_RULES = (
    ...
    (_MEMORY_CEILING_PATTERNS, _V_OVERLOADED),
    (_PAYLOAD_TOO_LARGE_PATTERNS, _V_PAYLOAD_TOO_LARGE),
    (_REQUEST_VALIDATION_PATTERNS, _V_FORMAT_ERROR),
    (_INVALID_MESSAGE_BODY_PATTERNS, _V_FORMAT_ERROR),
) + _IMAGE_TOOL_RULES
...
def _by_environmental_network(c: _Ctx) -> Optional[Verdict]:
    if (isinstance(c.error, OSError) or c.error_type in _TRANSPORT_ERROR_TYPES) and _is_environmental_network_failure(c.error, c.msg):
        return _v(_R.timeout, retryable=False, should_fallback=True)
    return None

# Stage order: plugin hooks → provider-specific special cases → HTTP status →
# MoA shapes → structured error code → environmental network → message patterns → SSL → disconnect +
...
_STAGES: Sequence[Callable[[_Ctx], Optional[Verdict]]] = (
    ...
    _by_error_code, _by_environmental_network, _by_message, _by_transport,
)
...
# read window). 504 Gateway Timeout routes to timeout so progressive backoff (#1100)
# and consecutive-timeout circuit breaker engage (#1142).
# Unlisted 4xx → format_error, 5xx → server_error.
_STATUS_HANDLERS: Dict[int, Callable[[_Ctx], Verdict]] = {
    ...
    403: _status_403, 404: _status_404, 408: lambda c: _V_TIMEOUT, 413: lambda c: _V_PAYLOAD_TOO_LARGE, 504: lambda c: _V_TIMEOUT,
