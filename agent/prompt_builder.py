...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    ...
    try:
        ...
        _record_context_load(str(soul_path), KIND_SOUL, loaded=False)
        return None
        ...
        for name in _AGENTS_MD_NAMES:
            ...
            if candidate.is_file():
                ...
                if section:
                    ...
                    _record_context_load(
                        str(candidate), KIND_AGENTS,
                        chars=len(_section_body(section)), section=section.split("\n", 1)[0],
                    )
        ...
        if candidate.exists():
            try:
                ...
                if content:
                    ...
                    _record_context_load(
                        str(candidate), KIND_CLAUDE,
                        chars=len(content), section=f"## {name}",
                    )
                    return result
            except Exception as e:
...
def _load_cursorrules(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    _record_context_load(
        str(cwd_path / ".cursorrules"), KIND_CURSORRULES, chars=len(cursorrules_content)
    )
    return result
