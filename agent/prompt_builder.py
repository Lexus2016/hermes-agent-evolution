...
def _scan_context_seams(seam_text: str, label: str) -> Optional[str]:
    """Catch a prompt injection split ACROSS the seam between concatenated context fragments."""
    ...
    return f"[BLOCKED: prompt injection detected in {label} (cross-source seam)]"
...
build_nous_subscription_prompt = None
...
def _hide_evolution_pipeline_skill(
    skill_name: str, session_platform: Optional[str] = None, frontmatter: Optional[dict] = None,
) -> bool:
...
def _build_skills_system_prompt_inner(
    ...
    def hides(frontmatter_name: str, skill_name: str, conditions: dict) -> bool:
        ...
        return (frontmatter_name in disabled or skill_name in disabled or _hide_evolution_pipeline_skill(frontmatter_name or skill_name, _platform_hint or None)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    ...
    try:
        ...
        if not content:
            return None
        _verify_prompt_integrity(soul_path.parent)
        res = _truncate_content(_scan_context_content(content, "SOUL.md"), "SOUL.md", context_length=context_length,
                                 read_path=str(soul_path))
        _record_context_load(str(soul_path), KIND_SOUL, chars=len(content), section="## SOUL.md")
        return res
    except Exception as e:
...
def _load_hermes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    res = _context_section(_strip_yaml_frontmatter(content), label, ".hermes.md", hermes_md_path, context_length)
    _record_context_load(str(hermes_md_path), KIND_HERMES, chars=len(content), section=f"## {label}")
    return res
...
def _load_agents_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """AGENTS.md — merged directory chain from git root down to cwd, plus AGENTS.override.md."""
    ...
    for directory in _dirs_root_to_cwd(cwd_resolved):
...
def _load_cursorrules(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    if not cursorrules_content:
        return ""
    res = _truncate_content(cursorrules_content, ".cursorrules", context_length=context_length,
                             read_path=str(cwd_path / ".cursorrules"))
    _record_context_load(str(cwd_path / ".cursorrules"), KIND_CURSORRULES, chars=len(cursorrules_content))
    return res
