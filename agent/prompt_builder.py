...
def _load_hermes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    for label, path, content in _hermes_md_candidates(cwd_path):
        if content:
            res = _context_section(_strip_yaml_frontmatter(content), label, ".hermes.md", path, context_length)
            _record_context_load(str(path), KIND_HERMES, chars=len(content), section=f"## {label}")
            return res
    return ""
