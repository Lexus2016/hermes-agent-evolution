...
def nonretryable_client_error_result(
    ...
    from agent.conversation_loop import (
        _billing_failure_result, _content_policy_blocked_result,
        _USER_ACTIONABLE_ABORT_REASONS, _user_actionable_provider_guidance,
    )
    ...
    if _welcome_hint:
        ...
        if classified.reason == FailoverReason.model_not_found and _prefix_suggestion:
    ...
    if classified.reason in _USER_ACTIONABLE_ABORT_REASONS:
        ...
        if classified.reason == FailoverReason.model_not_found and _prefix_suggestion:
            _actionable_final += (
                f"\nDid you mean '{_prefix_suggestion}'? It looks like the vendor prefix is missing."
            )
        result = _failed_turn_result(_actionable_final, messages, api_call_count, _nonretryable_summary)
        result.update({
...
def route_classified_error(
    ...
    if classified.reason == FailoverReason.overloaded:
        ...
        # #1205 — decay cross-turn streak on non-overloaded responses
    ...
    _should_fallback = (
        ...
        # instead of backoff-and-retry against the same degraded endpoint.
        or (
    ...
    if _should_fallback and agent._fallback_index < len(agent._fallback_chain):
        ...
        # credential-level. Bypass pool check for billing so fallback
        ...
        pool_may_recover = (
            False if _skip_pool_check else _ra()._pool_may_recover_from_rate_limit(agent._credential_pool)
        )
