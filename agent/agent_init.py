...
_CONTROL_STATE: Dict[str, Any] = {
    ...
    "_write_guard_policy": None,
    "_failure_recovery_enabled": False,
    "_plan_feasibility_enabled": False,
    "_plan_feasibility_report": None,
    "_failure_diagnosis_mode": "off",
    "_hypothesis_history": None,
    "_unhandled_drilldown": None,
    "_history_repaired_count": 0,
    "_save_session_log": True,
...
def _memory_provider_init_kwargs(agent, platform) -> Dict[str, Any]:
    ...
    if getattr(agent, "_user_id", None):
        kwargs["user_id"] = agent._user_id
    if getattr(agent, "_user_id_alt", None):
        kwargs["user_id_alt"] = agent._user_id_alt
    if getattr(agent, "_chat_id", None):
        kwargs["chat_id"] = agent._chat_id
    if getattr(agent, "_chat_name", None):
        kwargs["chat_name"] = agent._chat_name
    if getattr(agent, "_chat_type", None):
        kwargs["chat_type"] = agent._chat_type
    if getattr(agent, "_gateway_session_key", None):
        kwargs["gateway_session_key"] = agent._gateway_session_key
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    with suppress(Exception):
        agent._subagent_reground_interval = int(
            _agent_section.get("subagent_reground_interval", 10)
        )

    # Universal task-completion guidance toggle.  Default True.
    agent._task_completion_guidance = bool(
        _agent_section.get("task_completion_guidance", True)
    )

    # Universal parallel-tool-call guidance toggle.  Default True.
...
def init_agent(
    ...
    cwd: Optional[str] = None, write_guard_policy: Any = None,
):
    ...
    agent.user_id_alt = user_id_alt
    agent.user_name = user_name
    agent.chat_id = chat_id
    agent.chat_name = chat_name
    agent.chat_type = chat_type
    agent.thread_id = thread_id
    agent.gateway_session_key = gateway_session_key
    ...
    with suppress(Exception):
        _sess_cfg = _cfg_dict(_agent_cfg, "sessions")
