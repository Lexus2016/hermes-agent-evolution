...
def _review_tool_whitelist(
    ...
    """``(whitelist, configured_extra_tools)`` for the review fork (or Set[str] when called
    standalone for block_durable_writes assertion) — DISPATCH-side only, so the
    ...
    if review_agent is None:
        ...
        if block_durable_writes:
            names -= set(_DURABLE_WRITE_TOOLS)
        return names
    ...
    memory_on = getattr(review_agent, "_memory_enabled", False) or getattr(review_agent, "_user_profile_enabled", False)
    ...
    if block_durable_writes:
        whitelist -= set(_DURABLE_WRITE_TOOLS)
...
def _run_review_fork(
    ...
    explicit: bool = False, block_durable_writes: bool = False,
) -> None:
    ...
    if block_durable_writes:
        review_whitelist -= _DURABLE_WRITE_TOOLS
        configured_extra_tools = set()
...
def _run_review_in_thread(
    ...
    try:
        ...
        with thread_scoped_silence():
            _run_review_fork(agent, messages_snapshot, prompt, task_cfg, review_run, st, review_memory, explicit, block_durable_writes=block_durable_writes)
