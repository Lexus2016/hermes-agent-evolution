...
from agent.usage_pricing import CanonicalUsage, estimate_usage_cost, format_cost_label, format_duration_compact, format_token_count_compact, has_known_pricing
...
class InsightsEngine:
    ...
    _GET_PER_SESSION_TOOL_NAMES_ALL, _GET_PER_SESSION_TOOL_NAMES_WITH_SOURCE = _scoped(
    ...
    _GET_PER_SESSION_TOOL_CALLS_ALL, _GET_PER_SESSION_TOOL_CALLS_WITH_SOURCE = _scoped(
        "SELECT m.session_id, m.tool_calls" + _ASSISTANT_CALLS,
        " AND m.role = 'assistant' AND m.tool_calls IS NOT NULL",
    )
    ...
    _PINNED = ("_GET_TOOL_CALLS", "_GET_SKILL_CALLS", "_GET_PER_SESSION_TOOL_CALLS")
    ...
    def generate(self, days: int = 30, source: str = None) -> Dict[str, Any]:
        ...
        tool_costs = self._compute_tool_cost_attribution(sessions, per_session_tool_usage)
    ...
    def _get_per_session_tool_usage(
        ...
        for row in self._query("_GET_PER_SESSION_TOOL_NAMES", cutoff, source):
        ...
        for row in self._query("_GET_PER_SESSION_TOOL_CALLS", cutoff, source):
            try:
                for fn in _iter_functions(row["tool_calls"]):
                    name = fn.get("name")
                    if name:
    ...
    def _compute_tool_cost_attribution(
        ...
        for sid, usage in per_session_tool_usage.items():
            session = session_by_id.get(sid)
            if not session:
                continue

            estimated, _status = _estimate_cost(session)
