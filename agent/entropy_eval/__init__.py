...
class EntropyEngine:
    ...
    def analyze(
        ...
        *,
        ...
        actions = (
            _internal_actions
            if _internal_actions is not None
            else self._extract_actions(messages)
        )
        ...
        transition_events = sum(transitions.values())
        trajectory_entropy = _safe_entropy(transitions, transition_events)

        # 4. exploration_ratio = |unique transitions| / total_actions
        exploration_ratio = len(transitions) / max(total_actions, 1)
...
def format_report_terminal(report: SessionEntropyReport) -> str:
    ...
    if report.tool_entropy > 2.0:
        lines.append("  ⚠️  Tool entropy is high — agent uses many different tools.")
    if report.exploration_ratio < 0.1:
        lines.append("  💡 Exploration is low — very repetitive action pattern.")
    if report.exploration_ratio > 0.8:
        lines.append("  ⚠️  Exploration ratio is very high — chaotic or unfocused.")
    return "\n".join(lines)
