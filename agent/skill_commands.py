...
def _scan_skill_md(skill_md: Path, disabled: set, seen_names: set, commands: Dict[str, Dict[str, Any]], resolve_command, pending_colliding: list) -> None:
    ...
    try:
        ...
        for _iter in iters:
            for skill_md in _iter:
                try:
                    _scan_skill_md(skill_md, disabled, seen_names, commands, resolve_command, pending_colliding)
                except Exception:
                    continue
...
def resolve_skill_command_key(command: str) -> Optional[str]:
    ...
    Returns the matching /slug key from get_skill_commands() or None.
    """
    if not command:
        return None
