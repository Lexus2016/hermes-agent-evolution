...
def get_trust_promotion_threshold() -> int:
    ...
    return _config_number("trust_promotion_threshold", DEFAULT_TRUST_PROMOTION_THRESHOLD, int)


def get_trust_demotion_failure_rate() -> float:
    """Failure-rate threshold to demote trusted→provisional (#2256)."""
    return _config_number("trust_demotion_failure_rate", DEFAULT_TRUST_DEMOTION_FAILURE_RATE, float)
...
def apply_automatic_transitions(now: Optional[datetime] = None) -> Dict[str, int]:
    ...
    counts = {"marked_stale": 0, "archived": 0, "reactivated": 0, "checked": 0, "seeded": 0, "rationale_decayed": 0}
    ...
    for row in _u.curated_report():
        ...
        if getattr(_u, "rationale_decayed", None) and _u.rationale_decayed(row):
    ...
    *dry_run* SKIPS the stale/archive transitions and instructs the fork to report only; REPORT.md is still written and
    ...
    auto_summary = ", ".join(
        f"{counts[key]} {label}" for key, label in (
            ...
            ("rationale_decayed", "rationale-decayed (review needed)"),
        ) if counts.get(key)
    ) or "no changes" 
