...
def _sum_delegate_task(name, args, content, content_len, line_count):
    ...
    try:
        ...
        parsed = _json.loads(content) if content else None
...
# ---------------------------------------------------------------------------
# Slice A (#1761): Pinned-constraint contract + parsing.
#
# Context compaction drops turns across the conversation, which can silently
# discard safety and governance rules established early in the session (Governance
...
def _pinned_constraint_survives(
    ...
    """Return True if *constraint_text* is sufficiently preserved in *compressed_messages*."""
...
Write only the summary body. Do not include any preamble or prefix."""
    ...
    def compress(
        ...
        # Note: _demote_origin_for_spliced_summary(msg) is called when merging summary into tail.
...
def split_user_originated_turn(message: Any) -> tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    ...
    for key in (
        ...
        _DB_PERSISTED_MARKER, *(("_row_id",) if is_summary else ()), "display_kind", "display_metadata", "origin",
    ):
