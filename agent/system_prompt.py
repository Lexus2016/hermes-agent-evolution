...
def _guidance_parts(agent: Any) -> List[str]:
    ...
    # process_manage, write_file, patch, execute_code), enforce finishing the job with real tool output.
    ...
    if getattr(agent, "_task_completion_guidance", True) and agent.valid_tool_names:
        ...
        if _has_execution_or_write:
            ...
            parts.append(RESTRICTED_TOOLSET_GUIDANCE)
    ...
    parts.append(ATTENTION_RESET_GUIDANCE)
    ...
    if getattr(agent, "platform", None) == "cron":
        parts.append(DELIBERATE_WORK_GUIDANCE)
    ...
    parts.append(UNTRUSTED_CONTENT_GUIDANCE)
    ...
    if agent.valid_tool_names:
        parts.append(RECOVERY_BEFORE_REFUSAL_GUIDANCE)
    ...
    if getattr(agent, "_model_first_reasoning", False) and agent.valid_tool_names:
        parts.append(MODEL_FIRST_REASONING_GUIDANCE)
    ...
    if "compact_context" in agent.valid_tool_names:
        parts.append(SELFCOMPACT_RUBRIC_GUIDANCE)
    ...
    if any(str(n).startswith("mcp_tqmemory_") for n in agent.valid_tool_names):
        parts.append(TQMEMORY_GUIDANCE)
