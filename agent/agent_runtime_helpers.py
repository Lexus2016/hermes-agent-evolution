...
AGENT_RUNTIME_POST_HOOK_TOOL_NAMES = frozenset({
    ...
    "drive_preview", "annotate_preview", "read_window_below", "manage_connections", "setup_mcp",
    "gui_tour", "delegate_task", "compact_context",
})
...
def recover_with_credential_pool(
    ...
    if effective_reason == FailoverReason.billing:
        ...
        _ra().logger.info(
            "Credential pool exhausted (billing) for provider %s — "
...
def _api_error_debug_info(error: Exception) -> Dict[str, Any]:
    ...
    try:
        ...
        if classified:
            info["failure_category"] = getattr(classified.reason, "value", str(classified.reason))
            info["retryable"] = classified.retryable
    except Exception as e:
        _ra().logger.debug("Could not classify error for request dump: %s", e)
    return info
...
def dump_api_request_debug(
    ...
    try:
        ...
        if error is not None:
            ...
            _failure_category = dump_payload.get("error", {}).get("failure_category", "unknown")
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if inline_executor is not None:
        ...
        def _execute(next_args: dict) -> Any:
            # #2237 — replay-or-fork: a restored checkpoint must not duplicate
            # a non-atomic side effect. Fail-open: never block a tool on log
            # bookkeeping.
            try:
            ...
            # Late-import the facade so ``patch("tools.tool_call_log.get_default_log")`` hits.
            try:
            ...
            # Tests patch ``_ra`` to a stub; production reads model_tools so
            # in-tree code never goes through the run_agent compat pointer.
            ra = _ra()
            if getattr(ra, "__name__", None) != "run_agent":
                return ra.handle_function_call(
