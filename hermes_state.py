...
def _try_acquire_auto_maintenance_lock(db_path: Path) -> Optional[Any]:
    ...
    """
    if not hasattr(db_path, "with_name") or not hasattr(db_path, "open"):
        return type("DummyLock", (), {"close": lambda self: None})()
    lock_path = db_path.with_name(db_path.name + ".auto-maintenance.lock")
    try:
        if hasattr(lock_path, "parent") and hasattr(lock_path.parent, "mkdir"):
            lock_path.parent.mkdir(parents=True, exist_ok=True)
        handle = lock_path.open("a+b")
...
def _pread_db_header(db_path: Path, length: int) -> "Optional[bytes]":
    ...
    with _HEADER_PROBE_LOCK:
        ...
        if cached is not None and (cached[1], cached[2]) != (getattr(st, "st_dev", None), getattr(st, "st_ino", None)):
...
def _stat_db_file_identity(path: Path) -> "Optional[tuple]":
    ...
    if not getattr(st, "st_dev", None) or not getattr(st, "st_ino", None):
        return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def _db_file_was_replaced(self) -> bool:
        ...
        if not hasattr(self.db_path, "name"):
            return False
    ...
    def _get_db_file_size(self) -> int:
        """Return size of db_path on disk."""
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        try:
            ...
            # VACUUM when either (a) we freed rows this pass and the reclaimable
            # fraction (freelist_count / page_count) exceeds min_vacuum_freelist_ratio,
            # or (b) the DB file exceeds db_size_vacuum_threshold bytes (issue #2373:
            # monotonic DB growth past backup limits).
            # Both throttled to once every min_vacuum_interval_days.
            ...
            if threshold > 0:
                try:
                    db_over_size = (
                        self.db_path.exists()
                        and self._get_db_file_size() > threshold
                    )
                except OSError:
                    db_over_size = False
            ...
            if vacuum and vacuum_due and (pruned > 0 or db_over_size):
                ...
                should_vacuum = db_over_size or (ratio is None or ratio > min_vacuum_freelist_ratio)
                if should_vacuum:
                    try:
                        self.vacuum()
                    ...
                    logger.debug(
                        ...
                        ratio * 100.0 if ratio is not None else 0.0,
